...
import time, sys, os, csv, re, xlsxwriter, math, glob, arcpy
...
def summarizeGlobalData(inShp, idField, otherFiles=-1, type=['N','N','N','N','N','N','N','C'], clip=True, tempFolder="C:/Temp"):
    ...
    for inFidx in range(0, len(inFiles)):
        ...
        if curType == 'C':
            ...
            outTable = os.path.join(tempFolder, "%s_%s.dbf" % (fName.replace(".", "").replace(".tif", ""), fCount))
            fCount += 1
...
###extent - set the extent of the output map as an extent feature
###scale - set the scale of the map as a number
def createMapFromMXD_loop(mxd, outputImage, zoomLayer, layerName, visibility = [], statusDefinition={}, 
    extent="", scale=""):
    df = arcpy.mapping.ListDataFrames(mxd)[0]
    if extent != "":
        df.extent = extent
    ...
    with arcpy.da.SearchCursor(loopLyr, ["OID@", layerName]) as cur:
        for feat in cur:
            ...
            arcpy.mapping.ExportToPNG(mxd, outputImage.replace(".png", "_%s.png" % feat[1]))

    
#Calculate the intersecting inShapes that intersect inPolys, 
#   optional fields allow for intersecting values to be calculated
...
def pointsInPolygons(inPolys, inShapes, fieldName, attachFields, 
    inFields=["SCHNM","MAX","ES00POP"], outField="CityName",
    clipPts=False, clipFile="D:/Temp/pts.shp"):
