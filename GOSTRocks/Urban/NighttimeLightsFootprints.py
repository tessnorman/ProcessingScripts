...
###############################################################################
import arcpy, os, datetime, csv, sys, re, xlsxwriter, shutil, tempfile
...
def FPpostProcessing(docsFolder, mapsFolder, gisFolder, outDef, cntry, finalShape, newCities, thresh, popField = "ES00POP"):
    ...
    workbook = xlsxwriter.Workbook(outDef)
    writeWorkbookDictTitlePage(workbook)
    ...
    writeShapefileXLS(workbook, finalShape, "Extents", "pop", "D", ["GRIDCODE"], orderFields, {'raw1992':rawStyle})
    writeShapefileXLS(workbook, newCities, "Cities", popField, "D", cityIgnoreFields, orderFields)
    writeWorkbookCharts(workbook)
    workbook.close()
...
def writeWorkbookDictTitlePage(workbook):
    ...
    titlePage = workbook.add_worksheet("Title Page")
    titlePage.set_column(0,0,130)
    titlePage.write(0,0,"Global Nighttime Lights Urban Extents and Growth Product - Alpha Release",titleCenter)    
    titlePage.write(1,0,"",titleCenter)    
    titlePage.write(2,0,"South Asia Urban Team, GSURR: Mark Roberts and Mihir Prakash",titleFormat)
    titlePage.write(3,0,"In technical collaboration with ITSOP: Benjmain P Stewart and Katie McWilliams",titleFormat)
    titlePage.write(4,0,"",titleFormat)
    titlePage.write(5,0,"This Spreadsheet contains 5 pages", titleCenter)
    titlePage.write(6,0,"1. Title Page",titleFormat)
    titlePage.write(7,0,"2. Data Dictionary - Describes the column definitions in sheets 3 and 4",titleFormat)
    titlePage.write(8,0,"3. Urban Extents - Summary of urban areas extracted from radiance calibrated nighttime lights",titleFormat)
    titlePage.write(9,0,"4. Cities Inventory - Cities inventory with descriptive nighttime lights attributes (default Cities are from GRUMP 2000 populated places inventory)",titleFormat)
    titlePage.write(10,0,"5. Charts - Sample charts showing change in brightness in 10 largest urban extents.",titleFormat)
    
    #Write a Data Dictionary
    dataDict = workbook.add_worksheet("Data Dictionary")
    dataDict.write(0,1,"Extents and Cities Column Definitions", titleCenter)
    dataDict.write(0,0,"", titleCenter)
    dataDict.set_column(1,1,1200)
    dataDict.write_column(1,0,["FID","ExtentName","ExtTypeT0","CtyCntT0","ExtTypeT1","CtyCntT1","Status","gAreaKM","pop","RC1996_T0","RC2010_T1","ntlChange","ntlChgCorr","intensive","extensive","extenCorr","areaChg","raw1992","rc1996"])
    dataDict.write_column(1,1,["GIS Code for joining to Master Extents shapefiles",
...
def writeWorkbookCharts(workbook):
    extentChart = workbook.add_chart({'type':'column'})
    chartPage = workbook.add_worksheet("Charts")
    #Add data on NTL Change - Total, Intensive, and Extensive
    extentChart.add_series({'name':'NTL Change','values':'=Extents!$L$2:$L$10','categories':'=Extents!$B$2:$B$10'})
    extentChart.add_series({'name':'Intensive','values':'=Extents!$N$2:$N$10'})
    extentChart.add_series({'name':'Extensive','values':'=Extents!$O$2:$O$10'})
    #Set titles
    extentChart.set_title({'name':'Change in Extent Brightness'})
    extentChart.set_x_axis({'name':'10 largest cities'})
    extentChart.set_y_axis({'name':'Total change in brightness'})
    extentChart.set_size({'width':600,'height':600})
